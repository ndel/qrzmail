...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const list = db.prepare("SELECT id FROM marketing_lists WHERE id = ? AND owner_id = ?").get(list_id, user.id);
    if (!list) return NextResponse.json({ error: "List not found" }, { status: 404 });

    const insert = db.prepare(`
      INSERT OR IGNORE INTO marketing_contacts (id, owner_id, list_id, email, name, company, phone, custom_fields)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
