...
export function enqueueCampaign(campaignId: string): { enqueued: number; errors: string[] } {
  ...
  const insertQueue = db.prepare(
    "INSERT INTO marketing_queue (id, campaign_id, contact_id, provider_id, user_id, status, subject, html_body, plain_body, tracking_id) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)"
  );
  ...
  const transaction = db.transaction(() => {
    for (const contact of contacts) {
      try {
        ...
        insertQueue.run(queueId, campaignId, contact.id, campaign.provider_id, campaign.user_id, subject, htmlBody, plainBody || null, trackingId);
