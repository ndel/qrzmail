...
export async function GET() {
  ...
  const providers = db.prepare("SELECT * FROM marketing_providers WHERE user_id = ? ORDER BY created_at DESC").all(user.id);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO marketing_providers (id, user_id, name, smtp_host, smtp_port, smtp_user, smtp_pass, smtp_secure, imap_host, imap_port, imap_user, imap_pass, imap_secure, daily_limit, monthly_limit)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
